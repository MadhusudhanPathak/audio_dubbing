...
def validate_reference_audio_duration(
    file_path: str,
    ...
    Args:
        ...
        min_duration: Minimum duration in seconds (default: AppConfig.MIN_REF_AUDIO_DURATION)
        max_duration: Maximum duration in seconds (default: AppConfig.MAX_REF_AUDIO_DURATION)
    ...
    if min_duration is None:
        min_duration = config.MIN_REF_AUDIO_DURATION
    if max_duration is None:
        max_duration = config.MAX_REF_AUDIO_DURATION
...
def get_nllb_languages() -> dict:
    """
    Get supported NLLB-200 languages for dubbing (restricted set).
...
def get_supported_audio_formats() -> List[str]:
    ...
    """
    return list(get_config().SUPPORTED_AUDIO_FORMATS)
