...
class Transcriber:
    def __init__(self, model_path: str):
        ...
        # Log warning if Whisper.exe not found, but don't fail - it will fail at transcription time
        if not os.path.exists(self.whisper_exe):
            logging.warning(
                f"Whisper.exe not found at {self.whisper_exe}. "
                f"Please download it from https://github.com/ggerganov/whisper.cpp/releases "
